...
void test_push_pop(void)
{
	...
	for(int i=0; i<4; i++)
	{
		err = linklist_pop(&linklist_handler, &data, &len);
		if(err != kNoErr)
		{
			sys_log("linklist pop error");
			goto exit;
		}		
		sys_log("pop node is: %d, %s, left size is: %d", len, data, linklist_handler->node_size);
		free(data);
	}
...
void test_push_empty(void)
{
	...
	if(err != kNoErr)
	{
			sys_log("linklist empty error");
			goto exit;
	}
	sys_log("node size is: %d", linklist_handler->node_size);

exit:
	sys_log("######## Done !");
}

void test_push_deinit(void)
{
	OSStatus err = kNoErr;	
	linklist_handler_t linklist_handler = NULL;
	data_t * data = NULL;
	...
	err = linklist_deinit(&linklist_handler);
	if(err != kNoErr)
	{
		sys_log("linklist deinit error");
		goto exit;
	}
